...
function build(){
   for (let y = 0; y < new_map.length; y ++) {
    for (let x = 0; x < new_map[0].length; x ++){
      if(new_map[x][y] == new_map[x+1][y]){
        for(var x_counter = 0;new_map[x][y] == new_map[x+1][y]; x_counter++){
        }
        // [x, y, z, width, height, depth, material]
        items.push([j*UNIT*block_length, f*UNIT*block_height+2, i*UNIT*block_width, UNIT*block_width, UNIT*block_height, UNIT*block_length*x_counter, {color: 0x00FF00, side: THREE.DoubleSide}]);
      }
    }
   }
}
